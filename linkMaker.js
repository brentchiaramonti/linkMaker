    ...
    function processTheRow(row, dict, name, txt){
		...
		return "<label oncontextmenu='copyLink( \"" + name + "\", \"" + dict[text]["url"] +"\");return false;'><input type=\"checkbox\" name=\"" + name + "\" onclick='highlight(this)' '>" + text + "</label><br>";
...
function displayCheckboxes(text, dict, name, thisButton, target, prepend) {
	...
	searchBar.addEventListener("input", searchCheckboxes(searchBar, name));
